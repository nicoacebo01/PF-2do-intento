...
interface DebtByTypeDashboardProps {
  chartFilter: { type: string; value: string } | null;
  setChartFilter: (filter: { type: 'type'; value: string } | null) => void;
}

const DebtByTypeDashboard: React.FC<DebtByTypeDashboardProps> = ({ chartFilter, setChartFilter }) => {
  const { companyDebts, debtTypes, appSettings } = useFinancialCalculations();
  ...
  const handlePieClick = useCallback((data: any) => {
    const { name } = data;
    if (chartFilter && chartFilter.type === 'type' && chartFilter.value === name) {
        setChartFilter(null);
    } else {
        setChartFilter({ type: 'type', value: name });
    }
        ...
        <PieChart>
            ...
            {debtData.map((entry, index) => (
                <Cell 
                    ...
                    opacity={chartFilter && chartFilter.type === 'type' && chartFilter.value !== entry.name ? 0.5 : 1}
...
export default DebtByTypeDashboard;