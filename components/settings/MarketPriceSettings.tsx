...
interface MarketPriceSettingsProps {
...
const MarketPriceSettings: React.FC<MarketPriceSettingsProps> = ({ investments, priceHistory, onSavePrices, onDeleteMarketPriceSnapshot }) => {
  ...
  const handleExportHistory = () => {
    const flatData = priceHistory.flatMap(snapshot =>
        Object.entries(snapshot.prices).map(([instrument, price]) => ({
            date: snapshot.date,
            instrument,
            price
        }))
    ).sort((a, b) => b.date.localeCompare(a.date) || a.instrument.localeCompare(b.instrument));

    const columns: ExportColumn<typeof flatData[0]>[] = [
        { header: 'Fecha', accessor: d => d.date },
        { header: 'Instrumento', accessor: d => d.instrument },
        { header: 'Precio', accessor: d => d.price },
    ];

    exportToExcel(flatData, columns, 'historico_cotizaciones_mercado');
  };
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 bg-gray-100 dark:bg-gray-800/50 p-6 rounded-lg mb-6">
        <div>
      ...
      <div className="mt-8">
        <div className="flex justify-between items-center mb-4">
            <h3 className="text-xl font-semibold text-gray-700 dark:text-gray-200">Histórico de Cotizaciones Guardadas</h3>
